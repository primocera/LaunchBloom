...
Repository `primocera/LaunchBloom` · branch `v19` · generated 2026-08-16T00:00:00Z
...
- On disk: 41 files, range 001-037. 39 numbered migrations plus two unnumbered files that are deliberately NOT part of the applied set: CHECK_APPLIED.sql (a verification query) and E2E_MARKER.sql (the seeding opt-in, which must be run against a non-production database only). The applied range is still 001-037: 038_stripe_object_ownership.sql (SV-01/v20) and 039_stripe_ownership_enforcement.sql (SV-21-01/v21) are on disk but NOT YET APPLIED — both are owner-only, additive and reversible. 038 adds customers.app_user_id + the stripe_object_ownership legacy-map; 039 adds a partial UNIQUE index on customers(app_user_id) so onConflict:'app_user_id' becomes a safe canonical billing key. Their preflight/apply/verify/rollback are in each file header. Until the owner applies 038+039, backfills app_user_id, reconciles every ambiguous legacy-map row and sets STRIPE_OWNERSHIP_ENFORCED=1, the runtime keeps the existing price-only legacy fallback and both migrations stay pending owner actions (see docs/RUNBOOK_STRIPE_OWNERSHIP.md and GET /api/admin/readiness ownership.state).
