...
- Candidate SHA: `9be75d1b03d5e2cd59c6757c0b2903ef6f09e064` (frozen)
- HEAD now: `9be75d1b03d5e2cd59c6757c0b2903ef6f09e064`
...
| [35870080139](https://github.com/primocera/LaunchBloom/actions/runs/35870080139) | `rc/v24-final` | `9be75d1b03d5` | success | 2026-09-23T13:52:05Z → 2026-09-23T13:57:04Z | candidate-gate: success, authenticated-e2e: success, owner-live-checks: success |

### Drift from the reviewed baseline

| Commit | Subject | Already closes |
|---|---|---|
| `972f420` | feat(billing): complete the deferred Stripe ownership architecture (SV-01/v20) | v20 SV-01: lands the ownership-architecture trio the owner deferred at v19, additively and with zero runtime behaviour change (978/978 unit tests, incl. all 55 billing/isolation suites; lint 0/0; bundle index-Cq2NTdSE unchanged). ONE canonical typed service lib/stripe-ownership.js (typed states owned/foreign/legacy_mapped/legacy_price/ambiguous/unavailable) that webhooks.js, payments.js and the reconciler delegate to — email/price are never sufficient proof, a conflicting exact+foreign stamp is ambiguous and never adopted. Additive migration 038_stripe_object_ownership.sql (customers.app_user_id + stripe_object_ownership legacy map) with preflight/apply/verify/rollback, plus scripts/backfill-stripe-ownership.js (read-only inventory default, bounded --apply, resume cursor, PII-free receipt) and docs/RUNBOOK_STRIPE_OWNERSHIP.md. OWNER-ONLY, NOT RUN: apply migration 038; run the backfill and reconcile every ambiguous row; then sunset the price-only fallback (each fallback use is measured by legacy_price_ownership_fallback). This does NOT advance public_paid to full GO and does NOT freeze a new candidate — v20 changed backend code and added migration 038, so a new RC must be cut before the pinned candidate moves past 24d350c. |
...
|---|---|---|---|---|
| ESLint | `npm run lint` | passed in CI | `9be75d1` | yes |
| Unit / contract / safety tests | `npm test` | passed in CI | `9be75d1` | yes |
| Production build | `npm run build:app` | passed in CI | `9be75d1` | yes |
| Stale-bundle detection | `npm run check:app-fresh` | passed in CI | `9be75d1` | yes |
| Public browser journeys | `npm run test:e2e` | passed in CI | `9be75d1` | yes |
| Authenticated seeded browser matrix | `npm run test:e2e:auth` | passed in CI | `9be75d1` | yes |
| DOCX / PDF / ZIP structural validation and bounds | `npm run test:export` | passed in CI | `9be75d1` | yes |
| Production configuration gate | `npm run release:check` | observed in production | `9be75d1` | yes |
| Launch-state document integrity | `npm run launch:verify` | passed in CI | `9be75d1` | yes |
| Hero contrast and responsive layout | `npm test` | passed in CI | `9be75d1` | yes |
| React Router RSC advisory reachability guard | `npm run check:router` | passed in CI | `9be75d1` | yes |
...
- **code:** v18 commits are independent and additive; revert the offending commit and rebuild app/. The v18 gap work landed as one feature commit (3fff6c3, X01/S06/S13/S05/X02/X04/X06) plus the single-worker RC-runner change (017ece2); reverting either does not touch migrations or the config gate.
- **data:** No migration is introduced by v18 (git diff 64e7691..017ece2 over backend/migrations/ is empty). Nothing to roll back at the data layer.
...
- `docs/OWNER_HANDOFF_V15.md` — v15 owner handoff, pinned to the prior candidate b234dad. Superseded by the v16/v17 candidate SHA 64e7691 and now the prior candidate 017ece2 (this launch-state record); the canonical current truth is docs/LAUNCH_STATE.md, regenerated from this manifest. Kept as history.
