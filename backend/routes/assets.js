...
const { trackDeliverableProgress, trackChannelBreadth } = require('../lib/deliverables-track');
const {
  websiteKitSchema,
  emailFlowSchema,
  campaignEmailSchema,
...
router.post('/generate-website-kit', idempotent('generate-website-kit'), planGate('asset_generations'), async (req, res, next) => {
  try {
    ...
    // v10 SC-07: does this campaign now span three channel types?
    await trackChannelBreadth({ workspaceId: ws.id, userId: req.userId, campaignId: camp.campaign && camp.campaign.id });
